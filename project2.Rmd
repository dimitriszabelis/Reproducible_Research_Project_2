...
wind_vals <- c("High Wind", "Strong Wind", "Wind", "Wnd", 
...
winter_snow_vals <- c(
  ...
  "Lake-Effect Snow", "Snow", "Ice Storm", "Sleet",
  "Ice", "Glaze", "Mix", "Icy"
)
...
marine_coastal_vals <- c(
  ...
  "Sea", "Beach", "Tide", "Freezing Spray", "Tsunami", "Seiche",
...
recode_event_type <- function(event_type){
        fcase(
                event_type %chin% thunderstorm_vals,       "Thunderstorm",
...
exponent_to_multiplier <- function(exp){
        fcase(
                ...
                exp %chin% c("M", "m"), 1e6,
                exp %chin% c("B"), 1e9,
                default = NA # all digits from 0 to 8, "+" , "?", "-", & ""
        )
}
```
...
dt_check[ 
        ...
        , `:=`(
                event_type = str_extract(
                        event_type, 
                        regex(pattern, ignore_case = T)
                ), 
        ...
        complete.cases(dt_check[, .(total_property_damage, total_crop_damage)])
][
        , .(
                count = .N,
